...
contract Staking is ReentrancyGuard {
    ...
    function lockTokens(uint72 tokenAmount_, uint24 lockingPeriodInBlocks_) public poolTokenSet nonReentrant {
        require(block.number <= stakingProgramEndsBlock, "Staking Program has ended");
        require(lockingPeriodInBlocks_ > 0, "Locking period cannot be zero");
        // Convert blocks to days for easier handling of staking limits and calculations
        uint256 lockingPeriodInDays = lockingPeriodInBlocks_ / 28_721;
