...
contract StakingTest is Test {
    ...
    function test_lock_tokens_successfully_when_pool_is_set() public prank(recipient) {
        ...
        uint16 lockingPeriodInBlocks = 30;
        // Act
    ...
    function test_calculate_staking_reward_correctly() public {
        ...
        // Assert
        assertEq(expectedStakingRewardPoints, uint128((tokenAmount * staking.annualYieldRate() * lockingPeriod) / 365));
    }
...
contract TokenTest is Test {
    ...
    function setUp() public {
        ...
        poolToken.transfer(address(poolToken), 1e24);
    ...
    function test_call_public_vars() public prank(owner) {
        console.log("Token contract deployed at", address(poolToken));
        // Act
        poolToken.uniswapV2Router();

        poolToken.uniswapV2Pair();
        poolToken._taxFee();
    ...
    function test_swapping_token_for_bnb() public prank(owner) {
        // Preconditions
        // Act
        vm.expectRevert("PancakeLibrary: INSUFFICIENT_LIQUIDITY"); // will brick if liquidity has not been provided.
        poolToken.swapTokensForBNB(1e18);
        // Assert
    }
    ...
    function test_buying_tokens_with_bnb() public prank(owner) {
        // Preconditions
        // Act
        console.log("token total supply", poolToken.totalSupply());
        poolToken.buyTokensBNB{value: 1}();
        // Assert
    }

    function test_attempting_to_buy_with_unsupported_token(uint256 buyAmount) public prank(owner) {
        // Preconditions
    ...
    function test_buying_tokens_with_supported_tokens() public prank(owner) {
        // Preconditions

        // Act
        usdc.approve(address(poolToken), UINT256_MAX);
        poolToken.buyTokens(address(usdc), 1e2);
        // Assert
    }
}
